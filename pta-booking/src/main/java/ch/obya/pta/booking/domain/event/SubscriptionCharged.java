...
public record SubscriptionCharged(SubscriptionId subscription, ParticipantId participant, Instant timestamp) implements Event {
    public SubscriptionCharged {
        ifNullThrow(subscription, CommonProblem.AttributeNotNull.toException("SubscriptionCharged.subscription"));
        ifNullThrow(participant, CommonProblem.AttributeNotNull.toException("SubscriptionCharged.participant"));
        timestamp = ofNullable(timestamp).orElseGet(Instant::now);
    }

    public SubscriptionCharged(SubscriptionId subscription, ParticipantId participant) {
